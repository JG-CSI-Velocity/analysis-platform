...
class DCTRBranches(AnalysisModule):
        ...
        if charts_dir != ctx.paths.base_dir and not br_all.empty:
            ...
            try:
                ...
                with chart_figure(save_path=save_to) as (fig, ax):
                    ax.barh(dr["Branch"].astype(str), dr["DCTR %"] * 100, color=TEAL,
                    ...
                    for i, (d_val, t) in enumerate(zip(dr["DCTR %"], dr["Total Accounts"])):
                        ax.text(d_val * 100 + 0.5, i, f"{d_val:.1%} ({int(t):,})",
    ...
    def _branch_trend(self, ctx: PipelineContext) -> list[AnalysisResult]:
        ...
        export = merged[["Branch", "Historical DCTR %", "L12M DCTR %", "Change pp",
        ...
        if charts_dir != ctx.paths.base_dir:
            ...
            try:
                ...
                with chart_figure(figsize=(14, fig_h), save_path=save_to) as (fig, ax):
                    y = np.arange(n)
                    h = 0.35
                    ax.barh(y + h / 2, merged["Historical DCTR %"], h, label="Historical",
                            color=SILVER, edgecolor="black", linewidth=1.5)
                    ax.barh(y - h / 2, merged["L12M DCTR %"], h, label="TTM",
                            color=TEAL, edgecolor="black", linewidth=1.5)
                    ax.set_yticks(y)
                    ax.set_yticklabels(merged["Branch"].values, fontsize=18, fontweight="bold")
                    ax.set_xlabel("DCTR (%)", fontsize=20, fontweight="bold")
                    ax.set_title("Branch DCTR: Historical vs TTM", fontsize=24, fontweight="bold", pad=20)
                    ax.xaxis.set_major_formatter(FuncFormatter(lambda x, p: f"{x:.0f}%"))
        ...
        if charts_dir != ctx.paths.base_dir and not dr.empty:
            ...
            try:
                with chart_figure(figsize=(14, 8), save_path=save_to) as (fig, ax):
                    x = np.arange(len(dr))
                    ax.bar(x, dr["DCTR %"] * 100, color=TEAL, edgecolor="black", linewidth=1.5, alpha=0.9)
                    for i, (d_val, t) in enumerate(zip(dr["DCTR %"], dr["Total Accounts"])):
                        ax.text(i, d_val * 100 + 1, f"{d_val:.1%}", ha="center", fontsize=16, fontweight="bold")
                    ax.set_xticks(x)
                    ax.set_xticklabels(dr["Branch"].values, rotation=45, ha="right", fontsize=16)
                    ax.set_ylabel("DCTR (%)", fontsize=20, fontweight="bold")
                    ax.set_title("Branch DCTR -- Trailing Twelve Months", fontsize=24, fontweight="bold", pad=20)
    ...
    def _heatmap(self, ctx: PipelineContext) -> list[AnalysisResult]:
        ...
        if charts_dir != ctx.paths.base_dir:
            ...
            try:
                ...
                with chart_figure(figsize=(max(14, n_m * 1.2), fig_h), save_path=save_to) as (fig, ax):
