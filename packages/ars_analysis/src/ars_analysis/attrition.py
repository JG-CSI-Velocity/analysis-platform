...
from ars_analysis.chart_style import (
    PERSONAL, BUSINESS, HISTORICAL, TTM, POSITIVE, NEGATIVE,
    NEUTRAL, SILVER, TEAL, TITLE_SIZE, AXIS_LABEL_SIZE, DATA_LABEL_SIZE,
    TICK_SIZE, LEGEND_SIZE, ANNOTATION_SIZE, BAR_EDGE, BAR_ALPHA,
    PCT_FORMATTER,
)
...
BALANCE_ORDER = [
    'Negative', '$0', '$1-$499', '$500-$999', '$1K-$2.5K',
    '$2.5K-$5K', '$5K-$10K', '$10K+',
]
...
def run_attrition_1(ctx):
    ...
    try:
        ...
        if n_closed > 0 and 'Date Closed' in closed.columns:
            ...
            yearly = closed_yr.groupby('_close_year').size().reset_index(
                name='Closures')
        ...
        if sd and ed and n_closed > 0:
            ...
            l12m_open_start = len(all_data[
                (all_data['Date Closed'].isna()) |
                (all_data['Date Closed'] >= sd)])
        ...
        summary = pd.DataFrame([{
            ...
            'Metric': 'Open Accounts', 'Value': len(open_accts),
        }, {
            'Metric': 'Closed Accounts', 'Value': n_closed,
        }, {
            'Metric': 'All-Time Attrition Rate', 'Value': f'{overall_rate:.1%}',
        }, {
            'Metric': 'L12M Attrition Rate', 'Value': f'{l12m_rate:.1%}',
        }])
        _save(ctx, summary, 'ATTR-1-Summary', 'A9.1 Overall Attrition Rate',
              metrics={'Attrition Rate': f'{overall_rate:.1%}',
        ...
        insight = {
            ...
            'subtitle': (f'{overall_rate:.1%} overall attrition rate '
        ...
        ctx['results']['attrition_1'] = {
            'overall_rate': overall_rate, 'l12m_rate': l12m_rate,
            'total': total, 'closed': n_closed, 'yearly': yearly,
        }
        _report(ctx, f"   A9.1 complete -- {overall_rate:.1%} overall, "
                      f"{l12m_rate:.1%} L12M")
    except Exception as e:
...
def run_attrition_2(ctx):
    ...
    try:
        ...
        dur['Duration'] = pd.Categorical(dur['Duration'],
        ...
        chart_path = None
        fig, ax = _fig(ctx, 'single')
        bars = ax.barh(dur['Duration'].astype(str), dur['Count'],
                       color=TEAL, edgecolor=BAR_EDGE, alpha=BAR_ALPHA)
        for bar, pct in zip(bars, dur['Pct']):
            w = bar.get_width()
            ax.text(w + dur['Count'].max() * 0.02, bar.get_y() + bar.get_height() / 2,
                    f'{int(w):,} ({pct:.0%})', va='center',
                    fontsize=DATA_LABEL_SIZE, fontweight='bold')
        ax.set_title('Account Lifespan Before Closure', fontsize=TITLE_SIZE,
                     fontweight='bold', pad=15)
        ...
        _save(ctx, dur, 'ATTR-2-Duration', 'A9.2 Closure Duration',
        ...
        ctx['results']['attrition_2'] = {'duration': dur,
                                          'first_year_pct': first_year_pct}
...
def run_attrition_3(ctx):
    ...
    try:
        ...
        w = 0.35
        open_vals = [comp.loc[comp['Status'] == 'Open', m].values[0]
                     for m in plot_metrics]
        closed_vals = [comp.loc[comp['Status'] == 'Closed', m].values[0]
                       for m in plot_metrics]
        b1 = ax.bar(x - w / 2, open_vals, w, label='Open', color=POSITIVE,
                     edgecolor=BAR_EDGE, alpha=BAR_ALPHA)
        b2 = ax.bar(x + w / 2, closed_vals, w, label='Closed', color=NEGATIVE,
                     edgecolor=BAR_EDGE, alpha=BAR_ALPHA)
        for bars in [b1, b2]:
            for bar in bars:
                h = bar.get_height()
        ...
        ax.set_title('Open vs Closed Account Comparison', fontsize=TITLE_SIZE,
                     fontweight='bold', pad=15)
        ...
        _slide(ctx, 'A9.3 - Open vs Closed', {
            'title': 'Open vs Closed Account Comparison',
            'subtitle': (f'Open: {len(open_accts):,} accounts | '
                         f'Closed: {len(closed):,} accounts'),
            'slide_type': 'screenshot',
            'layout_index': 4,
            'chart_path': chart_path,
            'category': 'Attrition',
...
def run_attrition_4(ctx):
    ...
    try:
        ...
        chart_path = None
        fig, ax = _fig(ctx, 'single')
        ax.barh(branch_plot['Branch'].astype(str),
                branch_plot['Attrition Rate'] * 100,
                color=TEAL, edgecolor=BAR_EDGE, alpha=BAR_ALPHA)
        overall_rate = branch_df['Closed'].sum() / branch_df['Total'].sum()
        ax.axvline(overall_rate * 100, color=NEGATIVE, ls='--', lw=2,
                   label=f'Average: {overall_rate:.1%}')
        for i, (rate, ct) in enumerate(zip(branch_plot['Attrition Rate'],
                                            branch_plot['Total'])):
            ax.text(rate * 100 + 0.5, i, f'{rate:.1%} (n={ct:,})',
                    va='center', fontsize=DATA_LABEL_SIZE - 4)
        ax.set_title('Attrition Rate by Branch', fontsize=TITLE_SIZE,
                     fontweight='bold', pad=15)
        ...
        _save(ctx, branch_df, 'ATTR-4-Branch', 'A9.4 Attrition by Branch',
              metrics={'Branches': len(branch_df),
                       'Highest': f"{branch_df.iloc[-1]['Branch']}: "
                                  f"{branch_df.iloc[-1]['Attrition Rate']:.1%}"})

        _slide(ctx, 'A9.4 - Attrition by Branch', {
...
def run_attrition_5(ctx):
    ...
    try:
        ...
        prod_df = pd.DataFrame({
            'Total': total_by_prod, 'Closed': closed_by_prod,
        }).fillna(0).astype(int)
        ...
        ax.bar(x, prod_df['Attrition Rate'] * 100, color=TEAL,
               edgecolor=BAR_EDGE, alpha=BAR_ALPHA)
        for i, (rate, ct) in enumerate(zip(prod_df['Attrition Rate'],
                                            prod_df['Total'])):
            ax.text(i, rate * 100 + 0.5, f'{rate:.1%}',
                    ha='center', fontsize=DATA_LABEL_SIZE, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(prod_df['Prod Code'].astype(str),
                           fontsize=TICK_SIZE - 2, rotation=45, ha='right')
        ax.set_title('Attrition Rate by Product Code (Top 10)',
        ...
        _slide(ctx, 'A9.5 - Attrition by Product', {
...
def run_attrition_6(ctx):
    ...
    try:
        ...
        for btype in ['No', 'Yes']:
            ...
            rows.append({'Type': label, 'Total': total, 'Closed': n_closed,
                         'Open': total - n_closed, 'Attrition Rate': rate})
        ...
        bars = ax.bar(pb_df['Type'], pb_df['Attrition Rate'] * 100,
                      color=colors, edgecolor=BAR_EDGE, alpha=BAR_ALPHA,
                      width=0.5)
        for bar, row in zip(bars, pb_df.itertuples()):
            ...
            ax.text(bar.get_x() + bar.get_width() / 2, h + 0.5,
                    ...
                    ha='center', fontsize=DATA_LABEL_SIZE, fontweight='bold')
        ax.set_title('Attrition: Personal vs Business',
                     fontsize=TITLE_SIZE, fontweight='bold', pad=15)
        ax.set_ylabel('Attrition Rate (%)', fontsize=AXIS_LABEL_SIZE)
        ax.yaxis.set_major_formatter(PCT_FORMATTER)
        ax.tick_params(labelsize=TICK_SIZE)
        ...
        _slide(ctx, 'A9.6 - Personal vs Business', {
            'title': 'Personal vs Business Attrition',
            'subtitle': (f"Personal: {pb_df.iloc[0]['Attrition Rate']:.1%} | "
...
def run_attrition_7(ctx):
    ...
    try:
        ...
        closed_copy['_tenure_days'] = (
            closed_copy['Date Closed'] - closed_copy['Date Opened']).dt.days
        ...
        tenure_df = pd.DataFrame({
            'Total': total_by_tenure, 'Closed': closed_by_tenure,
        }).fillna(0).astype(int)
        tenure_df['Attrition Rate'] = tenure_df['Closed'] / tenure_df['Total']
        tenure_df.index = pd.CategoricalIndex(tenure_df.index,
        ...
        chart_path = None
        fig, ax = _fig(ctx, 'single')
        ax.bar(tenure_df['Tenure'].astype(str),
               tenure_df['Attrition Rate'] * 100,
               color=TEAL, edgecolor=BAR_EDGE, alpha=BAR_ALPHA)
        for i, (rate, ct) in enumerate(zip(tenure_df['Attrition Rate'],
                                            tenure_df['Total'])):
            ax.text(i, rate * 100 + 0.5, f'{rate:.1%}',
                    ha='center', fontsize=DATA_LABEL_SIZE, fontweight='bold')
        ax.set_title('Attrition Rate by Account Tenure',
                     fontsize=TITLE_SIZE, fontweight='bold', pad=15)
        ax.set_ylabel('Attrition Rate (%)', fontsize=AXIS_LABEL_SIZE)
        ax.yaxis.set_major_formatter(PCT_FORMATTER)
        ax.tick_params(labelsize=TICK_SIZE - 2)
        ...
        _slide(ctx, 'A9.7 - Attrition by Tenure', {
...
def run_attrition_8(ctx):
    ...
    try:
        ...
        bal_df = pd.DataFrame({
            'Total': total_by_bal, 'Closed': closed_by_bal,
        }).fillna(0).astype(int)
        bal_df['Attrition Rate'] = bal_df['Closed'] / bal_df['Total']
        bal_df.index = pd.CategoricalIndex(bal_df.index,
        ...
        chart_path = None
        fig, ax = _fig(ctx, 'single')
        ax.bar(bal_df['Balance Tier'].astype(str),
               bal_df['Attrition Rate'] * 100,
               color=TEAL, edgecolor=BAR_EDGE, alpha=BAR_ALPHA)
        for i, (rate, ct) in enumerate(zip(bal_df['Attrition Rate'],
                                            bal_df['Total'])):
            ax.text(i, rate * 100 + 0.5, f'{rate:.1%}',
                    ha='center', fontsize=DATA_LABEL_SIZE - 2, fontweight='bold')
        ax.set_title('Attrition Rate by Balance Tier',
                     fontsize=TITLE_SIZE, fontweight='bold', pad=15)
        ax.set_ylabel('Attrition Rate (%)', fontsize=AXIS_LABEL_SIZE)
        ax.yaxis.set_major_formatter(PCT_FORMATTER)
        ...
        _slide(ctx, 'A9.8 - Attrition by Balance', {
...
def run_attrition_9(ctx):
    ...
    try:
        ...
        for debit_val, label in [('Yes', 'With Debit Card'),
            ...
            rate = n_closed / total if total > 0 else 0
            rows.append({'Debit Status': label, 'Total': total,
                         'Closed': n_closed, 'Open': total - n_closed,
                         'Attrition Rate': rate})
        ...
        bars = ax.bar(debit_df['Debit Status'],
        ...
        for bar, row in zip(bars, debit_df.itertuples()):
            ...
            ax.text(bar.get_x() + bar.get_width() / 2, h + 0.5,
                    f'{row._5:.1%}\n({row.Closed:,} / {row.Total:,})',
                    ha='center', fontsize=DATA_LABEL_SIZE, fontweight='bold')
        ax.set_title('Debit Card Impact on Account Retention',
                     fontsize=TITLE_SIZE, fontweight='bold', pad=15)
        ax.set_ylabel('Attrition Rate (%)', fontsize=AXIS_LABEL_SIZE)
        ax.yaxis.set_major_formatter(PCT_FORMATTER)
        ax.tick_params(labelsize=TICK_SIZE)
        ...
        if retention_lift > 0:
            ax.annotate(f'{retention_lift:.1%} lower attrition\nwith debit cards',
                        xy=(0, with_rate * 100), fontsize=ANNOTATION_SIZE,
        ...
        _save(ctx, debit_df, 'ATTR-9-Debit', 'A9.9 Debit Card Retention',
              metrics={'With Debit': f'{with_rate:.1%}',
                       'Without Debit': f'{without_rate:.1%}',
                       'Retention Lift': f'{retention_lift:.1%}'})

        if retention_lift > 0:
            subtitle = (f'Accounts with debit cards show {retention_lift:.1%} '
                        f'lower attrition')
        else:
        ...
        ctx['results']['attrition_9'] = {
            'debit': debit_df, 'retention_lift': retention_lift}
...
def run_attrition_10(ctx):
    ...
    try:
        ...
        mail_cols = [c for c in all_data.columns
                     if re.match(r'^[A-Z][a-z]{2}\d{2}\s+Mail$', c)]
        resp_cols = [c for c in all_data.columns
                     if re.match(r'^[A-Z][a-z]{2}\d{2}\s+Resp$', c)]
        ...
        for grp in group_order:
            ...
            rate = n_closed / total if total > 0 else 0
            rows.append({'Group': grp, 'Total': total, 'Closed': int(n_closed),
                         'Attrition Rate': rate})
        ...
        bars = ax.bar(mail_df['Group'], mail_df['Attrition Rate'] * 100,
                      color=colors, edgecolor=BAR_EDGE, alpha=BAR_ALPHA,
                      width=0.5)
        for bar, row in zip(bars, mail_df.itertuples()):
            ...
            ax.text(bar.get_x() + bar.get_width() / 2, h + 0.5,
                    f'{row._4:.1%}\n({row.Closed:,} / {row.Total:,})',
                    ha='center', fontsize=DATA_LABEL_SIZE, fontweight='bold')
        ax.set_title('Mailer Program Impact on Retention',
                     fontsize=TITLE_SIZE, fontweight='bold', pad=15)
        ax.set_ylabel('Attrition Rate (%)', fontsize=AXIS_LABEL_SIZE)
        ax.yaxis.set_major_formatter(PCT_FORMATTER)
        ax.tick_params(labelsize=TICK_SIZE)
        ...
        if lift > 0:
            subtitle = (f'Mailer responders show {lift:.1%} lower attrition '
                        f'than never-mailed accounts')
        else:
...
def run_attrition_11(ctx):
    ...
    try:
        ...
        items_cols = sorted([c for c in closed.columns
                             if 'Items' in c or 'NSF' in c])

        closed_copy = closed.copy()
        if spend_cols:
            closed_copy['_last_spend'] = closed_copy[spend_cols].replace(
                0, np.nan).ffill(axis=1).iloc[:, -1].fillna(0)
        else:
            closed_copy['_last_spend'] = 0

        if items_cols:
            closed_copy['_last_items'] = closed_copy[items_cols].replace(
                0, np.nan).ffill(axis=1).iloc[:, -1].fillna(0)
        else:
        ...
        closed_copy['_est_annual_revenue'] = (
            ...
            closed_copy['_last_items'] * nsf_fee * 12
        )
        ...
        labels = ['$0-$50', '$50-$100', '$100-$250', '$250-$500',
                  '$500-$1K', '$1K+']
        closed_copy['_rev_bin'] = pd.cut(closed_copy['_est_annual_revenue'],
                                          bins=bins, labels=labels,
                                          include_lowest=True)
        rev_dist = closed_copy.groupby('_rev_bin', observed=True).agg(
            Count=('_rev_bin', 'size'),
            Total_Revenue=('_est_annual_revenue', 'sum')
        ).reset_index()

        fig, ax = _fig(ctx, 'single')
        ax.bar(rev_dist['_rev_bin'].astype(str), rev_dist['Total_Revenue'],
               color=NEGATIVE, edgecolor=BAR_EDGE, alpha=BAR_ALPHA)
        for i, (rev, ct) in enumerate(zip(rev_dist['Total_Revenue'],
                                           rev_dist['Count'])):
            ax.text(i, rev + total_lost * 0.02,
                    f'${rev:,.0f}\n({ct:,} accts)',
                    ha='center', fontsize=DATA_LABEL_SIZE - 2, fontweight='bold')
        ax.set_title('Estimated Annual Revenue Lost by Tier',
                     fontsize=TITLE_SIZE, fontweight='bold', pad=15)
        ...
        _save(ctx, summary, 'ATTR-11-Revenue', 'A9.11 Revenue Impact',
              metrics={'Annual Revenue Lost': f'${total_lost:,.0f}'})

        insight = {
            'title': 'Revenue Impact of Account Closures',
            'subtitle': (f'Est. ${total_lost:,.0f} in annual revenue lost '
                         f'from {n_closed:,} closed accounts'),
            'slide_type': 'screenshot_kpi',
            'layout_index': 5,
            'kpis': {
...
def run_attrition_12(ctx):
    ...
    try:
        ...
        monthly = l12m_closed.groupby('_close_month').size().reset_index(
            name='Closures')
        ...
        ax.bar(x, monthly['Closures'], color=TEAL, edgecolor=BAR_EDGE,
               alpha=BAR_ALPHA, label='Monthly Closures')
        ax.plot(x, monthly['MA3'], color=NEGATIVE, linewidth=3,
                marker='o', markersize=8, label='3-Mo Avg')
        for i, c in enumerate(monthly['Closures']):
            ax.text(i, c + monthly['Closures'].max() * 0.03, str(int(c)),
                    ha='center', fontsize=DATA_LABEL_SIZE - 2, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(month_labels, fontsize=TICK_SIZE - 2, rotation=45,
                           ha='right')
        ax.set_title('Monthly Account Closures (L12M)',
                     fontsize=TITLE_SIZE, fontweight='bold', pad=15)
        ax.set_ylabel('Closures', fontsize=AXIS_LABEL_SIZE)
        ax.legend(fontsize=LEGEND_SIZE)
        plt.tight_layout()
        ...
        ctx['results']['attrition_12'] = {
            'monthly': export_df, 'total_l12m': total_l12m, 'trend': trend}
        _report(ctx, f"   A9.12 complete -- {total_l12m:,} L12M closures, "
                      f"trend: {trend}")
    except Exception as e:
...
def run_attrition_13(ctx):
    ...
    try:
        ...
        all_copy['_ars_eligible'] = (
            all_copy['Stat Code'].isin(esc) & all_copy['Prod Code'].isin(epc))
        ...
        for elig, label in [(True, 'ARS-Eligible'), (False, 'Non-Eligible')]:
            ...
            n_closed = subset['Date Closed'].notna().sum()
            rate = n_closed / total if total > 0 else 0
            rows.append({'Group': label, 'Total': total,
                         'Closed': int(n_closed), 'Attrition Rate': rate})
        ...
        bars = ax.bar(ars_df['Group'], ars_df['Attrition Rate'] * 100,
                      color=colors, edgecolor=BAR_EDGE, alpha=BAR_ALPHA,
                      width=0.5)
        for bar, row in zip(bars, ars_df.itertuples()):
            ...
            ax.text(bar.get_x() + bar.get_width() / 2, h + 0.5,
                    f'{row._4:.1%}\n({row.Closed:,} / {row.Total:,})',
                    ha='center', fontsize=DATA_LABEL_SIZE, fontweight='bold')
        ax.set_title('Attrition: ARS-Eligible vs Non-Eligible',
                     fontsize=TITLE_SIZE, fontweight='bold', pad=15)
        ax.set_ylabel('Attrition Rate (%)', fontsize=AXIS_LABEL_SIZE)
        ax.yaxis.set_major_formatter(PCT_FORMATTER)
        ax.tick_params(labelsize=TICK_SIZE)
        ...
        if diff > 0:
            subtitle = (f'ARS-eligible accounts show {diff:.1%} lower '
                        f'attrition than non-eligible')
        else:
            subtitle = 'Attrition comparison by eligibility status'

        _slide(ctx, 'A9.13 - ARS vs Non-ARS', {
...
def run_attrition_suite(ctx):
    ...
    for fn in [
        run_attrition_1, run_attrition_2, run_attrition_3,
        run_attrition_4, run_attrition_5, run_attrition_6,
        run_attrition_7, run_attrition_8, run_attrition_9,
        run_attrition_10, run_attrition_11, run_attrition_12,
        run_attrition_13,
    ]:
        ctx = fn(ctx)
