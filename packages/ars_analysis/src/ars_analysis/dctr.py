    ...
    try:
        ...
        if not dr.empty:
            ...
            for i, v in enumerate(volumes):
                if v > 0:
                    ax2.text(
                        i, v * 0.05, f"{v:,}", ha="center", va="bottom", fontsize=16, color="gray"
                    )
...
def run_dctr_11(ctx):
    ...
    try:
        ...
        ax.set_title(
            "Eligible Accounts DCTR by Account Holder Age", fontsize=24, fontweight="bold", pad=25
        )
...
def run_dctr_funnel(ctx):
    ...
    try:
        ...
        for i, stage in enumerate(stages):
            width = (
                max_width * (stage["total"] / stages[0]["total"]) if stages[0]["total"] > 0 else 0.1
            )
            ...
            if has_biz and stage["total"] > 0:
                ...
                ax.text(
                    ...
                    color="black",
                    bbox=dict(
                        boxstyle="round,pad=0.4", facecolor="white", edgecolor="black", alpha=0.9
                    ),
                )
            else:
        ...
        if has_biz:
            ...
            ax.legend(
                handles=legend_elements, loc="lower right", fontsize=16, frameon=True, fancybox=True
            )
...
def run_dctr_l12m_trend(ctx):
    ...
    try:
        ...
        bars = ax2.bar(
            x, total_volume, alpha=0.25, color="gray", edgecolor="darkgray", linewidth=1, width=0.8
        )
...
def run_dctr_l12m_funnel(ctx):
    ...
    try:
        ...
        tp_a = (
            len(l12m_all[l12m_all["Business?"] == "No"]) if "Business?" in l12m_all.columns else ta
        )
        tb_a = ta - tp_a
        op_a = (
...
def run_dctr_decade_pb(ctx):
    ...
    try:
        ...
        ax.set_title(
            "Eligible Personal vs Business DCTR by Decade", fontsize=24, fontweight="bold", pad=20
        )
