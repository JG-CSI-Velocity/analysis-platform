...
def run_market_reach(ctx):
    ...
    # Outer circle
    outer = plt.Circle(
        (cx, cy), r_outer, facecolor=COLOR_OUTER, alpha=0.25, edgecolor=COLOR_OUTER, linewidth=2.5
    )
    ax.add_patch(outer)

    # Inner circle
    inner = plt.Circle(
        (cx, cy), r_inner, facecolor=COLOR_INNER, alpha=0.35, edgecolor=COLOR_INNER, linewidth=2.5
    )
    ax.add_patch(inner)
    ...
    for i, (val, label) in enumerate(kpis):
        y = 1.8 - i * 1.3
        ax.text(
            kpi_x, y, val, ha="left", va="center", fontsize=24, fontweight="bold", color="#1E3D59"
        )
        ax.text(kpi_x, y - 0.4, label, ha="left", va="center", fontsize=14, color="#555")
...
def run_spend_share(ctx):
    ...
    resp_pct_of_eligible = spend_responders / spend_eligible * 100 if spend_eligible > 0 else 0
    resp_pct_of_open = spend_responders / spend_all_open * 100

    # --- Draw chart: horizontal bars descending, responder bar highlighted ---
    fig, (ax, ax2) = plt.subplots(1, 2, figsize=(14, 7), gridspec_kw={"width_ratios": [1.4, 1]})
    fig.patch.set_facecolor("white")
    ...
    y_pos = [2, 1, 0]
    bars = ax.barh(y_pos, values, color=bar_colors, height=0.6, edgecolor="none", alpha=0.85)
    ...
    ax.set_xlabel(f"Total Spend ({latest_month})", fontsize=14, fontweight="bold")
    ...
    kpi_items = [
        ("Eligible Share of Open Spend", f"{elig_pct_of_open:.1f}%", "#1E3D59", 2.4),
        ("Responder Share of Eligible Spend", f"{resp_pct_of_eligible:.1f}%", COLOR_INNER, 1.4),
        ("Responder Share of All Open Spend", f"{resp_pct_of_open:.1f}%", COLOR_INNER, 0.4),
    ]
    for label, val, color, y in kpi_items:
        ax2.text(0.1, y + 0.15, label, ha="left", va="center", fontsize=12, color="#555")
        ax2.text(
            0.1, y - 0.15, val, ha="left", va="center", fontsize=26, fontweight="bold", color=color
        )
    ...
    subtitle = (
        f"Responders account for {resp_pct_of_eligible:.0f}% of eligible spend ({latest_month})"
    )
...
def run_revenue_attribution(ctx):
    ...
    bars = ax1.barh([1, 0], values, color=colors, height=0.5, edgecolor="none", alpha=0.9)
    ...
    ax1.set_yticks([0, 1])
    ax1.set_yticklabels(["Non-Responders", "Responders"], fontsize=14, fontweight="bold")
    ...
    for label, val, y in kpi_data:
        ax2.text(0.1, y + 0.05, label, ha="left", va="center", fontsize=13, color="#555")
        color = "#1E3D59" if "Incremental" not in label else COLOR_RESP
        ax2.text(
            0.1, y - 0.05, val, ha="left", va="center", fontsize=20, fontweight="bold", color=color
        )
...
def run_pre_post_delta(ctx):
    ...
    bars_pre = ax.bar(
        x - bar_w / 2, pre_vals, bar_w, color="#BDC3C7", edgecolor="none", label="Before Mailer"
    )
    bars_post = ax.bar(
        x + bar_w / 2,
        post_vals,
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(["Responders", "Non-Responders"], fontsize=18, fontweight="bold")
    ...
    if resp_delta > 0 and non_delta <= 0:
        framing = (
            "Responders grew while non-responders declined "
            "-- the program is protecting and growing revenue."
        )
    elif resp_delta > non_delta > 0:
        framing = (
            f"Both groups grew, but responders outpaced by ${resp_delta - non_delta:,.0f}/account."
        )
    elif resp_delta > non_delta:
        framing = (
            "Responder activity held stronger than non-responders "
    ...
    ax.text(
        0.5,
        -0.22,
        f"Net program lift: {nl_sign}${net_lift:,.0f}/account (after adjusting for market trends)",
        transform=ax.transAxes,
        ha="center",
        va="top",
        fontsize=12,
