...
def _calc_nu_metrics(data, resp_col, mail_col, metric_cols):
    ...
    return {
        'num_resp': n_resp, 'num_non_resp': n_non,
        'avg_resp': avg_resp, 'avg_non_resp': avg_non,
    }
...
def _draw_nu_chart(ax, dates, nu_metrics, metric_type, month):
    ...
    ax.plot(dates, avg_r.values, marker='o', color=COLOR_SCHEME['NU 5+'],
            linewidth=2.5, markersize=8, label='NU 5+ Responders')
    ax.plot(dates, avg_n.values, marker='s', color=COLOR_SCHEME['Non-Responders'],
            linestyle='--', linewidth=2, markersize=6, alpha=0.8, label='NU Non-Responders')
...
def _draw_th_chart(ax, dates, th_metrics, metric_type, month):
    ...
    for seg in TH_SEGMENTS:
        if seg in th_metrics:
            ...
            ax.plot(dates, avg.values, marker='o', color=COLOR_SCHEME[seg],
                    linewidth=2.5, markersize=8, label=seg)

    if 'TNR' in th_metrics:
        avg_tnr = th_metrics['TNR']['avg']
        ax.plot(dates, avg_tnr.values, marker='s', color=COLOR_SCHEME['Non-Responders'],
                linestyle='--', linewidth=2, markersize=6, alpha=0.8, label='TH Non-Resp')
...
def run_month_analysis(ctx, month, resp_col, mail_col, spend_cols, swipe_cols):
    ...
    _report(ctx, f"      NU Responders: {nu_spend['num_resp']:,}  |  Non-Resp: {nu_spend['num_non_resp']:,}")
    ...
    if all_results:
        try:
            ...
            for month, r in all_results.items():
                row = {'Month': month, 'NU Responders': r['nu_resp'],
            ...
            _save(ctx, summary_df, 'A12-MailInsights',
