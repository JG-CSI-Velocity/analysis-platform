...
def _build_insights(ctx, data, resp_col, seg_details, total_mailed, total_resp, overall_rate):
...
def _render_donut_chart(seg_details, chart_path, month_title=None):
    ...
    if total > 0:
        ...
        ax.text(
            0, 0.06, "Total", ha="center", va="center", fontsize=18, fontweight="bold", color="#555"
        )
        ax.text(0, -0.12, f"{total:,}", ha="center", va="center", fontsize=28, fontweight="bold")
    else:
        ax.text(
            0.5, 0.5, "No Responders", ha="center", va="center", fontsize=16, transform=ax.transAxes
        )
        ax.axis("off")
...
def _render_3col(seg_details, insights, title, chart_path):
    ...
    ax3.text(0.50, 0.93, "Key Insights", ha="center", va="top", fontsize=18, fontweight="bold")

    y = 0.83
    spacing = 0.085
    for i, txt in enumerate(insights[:8]):
        ax3.text(0.08, y - i * spacing, f"• {txt}", ha="left", va="top", fontsize=13, wrap=True)
...
def run_monthly_summaries(ctx):
    ...
    for month, resp_col, mail_col in pairs:
        ...
        seg_details, total_mailed, total_resp, overall_rate = _analyze_month(
    ...
    try:
        ...
        for seg in MAILED_SEGMENTS:
            if trend[seg] and len(trend[seg]) == len(months):
                ...
                ax.plot(
                    x, trend[seg], marker="o", color=color, linewidth=2.5, markersize=8, label=label
                )

        ax.set_xticks(x)
        ax.set_xticklabels(months, fontsize=16, fontweight="bold", rotation=45, ha="right")
        ...
        _slide(
            ...
            "A13.6 - Response Rate Trend",
            {
                ...
                "chart_path": cp,
                "insights": [
                    f"{'NU 5+' if s == 'NU' else s}: {trend[s][-1]:.1f}% (latest)"
    ...
    try:
        ...
        _slide(
            ...
            "A13.5 - Responder Count Trend",
            {
                ...
                "chart_path": cp,
                "insights": [
                    f"Latest: {latest_total:,} responders",
...
def run_account_age(ctx):
    ...
    if not all_rows:
        _report(ctx, "   ⚠️ No responders with valid dates")
        return ctx
    ...
    try:
        ...
        bars = ax.bar(
            x, cts, color=BAR_COLORS[: len(lbl_list)], edgecolor="black", linewidth=2, alpha=0.8
        )
        ...
        ax.set_title("Responder Distribution by Account Age", fontsize=20, fontweight="bold")
        ax.tick_params(axis="y", labelsize=14)
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
        ...
        ins = [
            f"Total: {grand:,}",
            f"Campaigns: {len(all_rows)} months",
...
def run_mailer_response_suite(ctx):
    ...
    slides = len(
        [
            s
            for s in ctx["all_slides"]
            if s["category"] == "Mailer" and s["id"].startswith(("A13", "A14"))
        ]
    )
