...
def run_pipeline(
    ...
    _report(ctx, "=" * 60)
    _report(ctx, "🚀 ARS PIPELINE v1.1.0 — STARTING")
    _report(ctx, f"   Python: {sys.executable}")
    _report(ctx, f"   Package: {Path(__file__).parent}")
    if ".venv" not in sys.executable.replace("\\", "/"):
        _report(ctx, "   ⚠️  Not using project .venv — try: uv run python -m ars_analysis.pipeline")
    _report(ctx, "=" * 60)
