...
from ars_analysis.ars_config import PRESENTATIONS_PATH, ARCHIVE_PATH, TEMPLATE_PATH
...
def step_parse_and_paths(ctx, base_paths=None, metadata=None):
    ...
    pptx_output_path = str(source_folder / f"{client_id}-{year}-{month}-{client_name}-presentation.pptx")
    pptx_archive_path = str(month_folder / f"{client_id}-{year}-{month}-{client_name}-presentation.pptx")
...
def step_load_config(ctx, config_path=None):
    ...
    _report(ctx, f"   Stat codes: {len(ctx['eligible_stat_code'])}  |  Prod codes: {len(ctx['eligible_prod_code'])}")
    return ctx
...
def step_date_range(ctx):
    ...
    last_12_months = pd.date_range(start=start_date, end=end_date, freq='ME').strftime('%b%y').tolist()

    ctx['start_date'] = start_date
    ctx['end_date'] = end_date
    ctx['last_12_months'] = last_12_months

    _report(ctx, f"📅 Analysis Period: {start_date.strftime('%B %Y')} – {end_date.strftime('%B %Y')} ({len(last_12_months)} months)")
    return ctx
...
class ARSFieldFormatter:
    FORMATTING_RULES = {
        'currency': {'patterns': ['$', 'spend', 'bal', 'limit', 'fee', 'amount', 'revenue'], 'excel_format': '$#,##0.00'},
        'integer': {'patterns': ['swipes', '#', 'count', 'items', 'mtd', 'transactions'], 'excel_format': '#,##0'},
        'percentage': {'patterns': ['rate', '%', 'percent', 'ratio', 'pct', 'dctr'], 'excel_format': '0.00%'},
        'decimal': {'patterns': ['age', 'years', 'months', 'days', 'score'], 'excel_format': '0.00'},
        'text': {'patterns': ['id', 'code', 'desc', 'mask', 'name', 'branch', 'type'], 'excel_format': '@'},
...
def write_formatted_df(ws, df, row, wb, sheet_title=None, key_metrics=None):
    ...
    for ci in range(1, len(df.columns) + 1):
        ...
        max_len = max(len(str(df.columns[ci-1])),
                      max((len(str(ws.cell(row=r, column=ci).value or ''))
                           for r in range(row+1, end_row+1)), default=0))
...
def save_to_excel(ctx, df, sheet_name, analysis_title, key_metrics=None):
    ...
    for file_path in [ctx['source_excel_path'], ctx['global_report_excel_file_path']]:
        ...
        if isinstance(df, dict):
            ...
            for sub_name, sub_df in df.items():
                if sub_df is not None and isinstance(sub_df, pd.DataFrame) and not sub_df.empty:
                    end = write_formatted_df(ws, sub_df, cur_row, wb,
        ...
        if 'Summary' not in wb.sheetnames:
            ...
            sws = wb['Summary']
        nr = sws.max_row + 1
        rc = len(df) if isinstance(df, pd.DataFrame) else sum(len(v) for v in df.values() if isinstance(v, pd.DataFrame))
...
def step_create_subsets(ctx):
    ...
    em = ctx['eligible_mailable'] if isinstance(ctx['eligible_mailable'], list) else [ctx['eligible_mailable']]
    ...
    # Eligible
    eligible_data = data[
        (data['Date Closed'].isna()) & (data['Stat Code'].isin(esc)) &
        (data['Prod Code'].isin(epc)) & (data['Mailable?'].isin(em)) &
        (data['Avg Bal'] != 0)
    ].copy()
    ...
    _report(ctx, f"   With debit: {len(eligible_with_debit):,}  |  Without: {len(eligible_without_debit):,}")
    _report(ctx, f"   Personal: {len(eligible_personal):,}  |  Business: {len(eligible_business):,}")
    ...
    if lrc:
        ...
        reg_e_eligible_base_l12m = eligible_personal_last_12m[eligible_personal_last_12m['Debit?'] == 'Yes'].copy()
        _report(ctx, f"   Reg E: {len(reg_e_opted_in):,} opted in / {len(reg_e_opted_out):,} opted out")

    ctx.update({
        'open_accounts': open_accounts, 'closed_accounts': closed_accounts,
        'eligible_data': eligible_data,
        'eligible_personal': eligible_personal, 'eligible_business': eligible_business,
        'eligible_with_debit': eligible_with_debit, 'eligible_without_debit': eligible_without_debit,
        'eligible_personal_with_debit': eligible_personal_with_debit,
        'eligible_business_with_debit': eligible_business_with_debit,
        'open_personal': open_personal, 'open_business': open_business,
        ...
        'reg_e_opted_in': reg_e_opted_in, 'reg_e_opted_out': reg_e_opted_out,
    })
    return ctx
...
def build_slide_content(ctx, slide_data, default_layout=4):
    ...
    if slide_data.get('slide_type') == 'mailer_summary':
        ...
        return SC(slide_type='mailer_summary', title=title,
                  images=[donut, hbar], kpis=kpis,
                  bullets=bullets, layout_index=layout_index)

    # --- Multi-screenshot (two charts side by side) ---
    if slide_data.get('slide_type') == 'multi_screenshot':
        imgs = [slide_data.get('chart_path', ''),
                slide_data.get('chart_path_2', '')]
        return SC(slide_type='multi_screenshot', title=full_title,
                  images=imgs, layout_index=layout_index)
    ...
    if kpis:
        return SC(slide_type='screenshot_kpi', title=full_title,
                  images=[chart_path], kpis=kpis, layout_index=layout_index)
    else:
        return SC(slide_type='screenshot', title=full_title,
                  images=[chart_path], layout_index=layout_index)
...
def _build_preamble_slides(ctx):
    ...
    preamble = [
        ...
        # 2. Agenda — uses dedicated Agenda layout
        SC(slide_type='blank',
           title='Agenda',
           layout_index=14),
...
def _reorder_analysis_slides(ctx, selected):
    ...
    # ------------------------------------------------------------------
    DCTR_MERGES = [
        ('A7.6a - Last 12 Months DCTR Trend', 'A7.4 - Segment Trends',
         'DCTR Trajectory: Recent Trend & Segments'),
        ('A7.7 - Historical Funnel', 'A7.8 - L12M Funnel',
         'DCTR Funnel: Historical vs TTM'),
        ('A7.11 - DCTR by Account Holder Age', 'A7.12 - DCTR by Account Age',
         'DCTR Opportunity: Age Analysis'),
    ]
    DCTR_APPENDIX_IDS = {
        'A7.5 - Decade Trend', 'A7.6b - Personal vs Business by Decade',
        'A7.13 - Monthly Heatmap', 'A7.14 - Seasonality',
        'A7.15 - Vintage & Cohort', 'A7.16 - Branch L12M Table',
        'A7.9 - Eligible vs Non-Eligible DCTR',
        'A7.10b - Branch DCTR (L12M Focus)', 'A7.10c - Branch Top 10',
    }

    REGE_MERGES = [
        ('A8.10 - Reg E All-Time Funnel', 'A8.11 - Reg E L12M Funnel',
         'Reg E Funnel: All-Time vs TTM'),
        ('A8.5 - Reg E by Account Age', 'A8.6 - Reg E by Holder Age',
         'Reg E Opportunity: Age Analysis'),
    ]
    REGE_APPENDIX_IDS = {
        'A8.7 - Reg E by Product', 'A8.4c - Reg E Branch Scatter',
        'A8.2 - Reg E Historical', 'A8.1 - Reg E Overall Status',
        'A8.12 - Reg E Trend',
    ...
    ATTRITION_MERGES = [
        ('A9.3 - Open vs Closed', 'A9.6 - Personal vs Business',
    ...
    _report(ctx, f"   Consolidated DCTR: {len(dctr)} -> {len(dctr_main)} main + {len(dctr_appendix)} appendix")
    _report(ctx, f"   Consolidated Reg E: {len(reg_e)} -> {len(rege_main)} main + {len(rege_appendix)} appendix")
    _report(ctx, f"   Consolidated Attrition: {len(attrition)} -> {len(attrition_main)} main + {len(attrition_appendix)} appendix")

    # Build subtitle for section dividers: "Client Name | Mon YYYY"
    month_name = calendar.month_name[int(ctx['month'])]
    section_subtitle = f"{ctx['client_name']} | {month_name} {ctx['year']}"

    def _section(title, layout_index=1, slide_type='title', kpis=None,
                 subtitle=None):
        """Create a section divider marker dict."""
        full_title = f"{title}\n{subtitle}" if subtitle else title
        return {'__section__': True, 'id': f'__section__{title}',
                'slide_type': slide_type, 'title': full_title,
                'layout_index': layout_index, 'kpis': kpis or {}}

    ordered = list(recent_clusters)

    if dctr_main or value_dctr:
        ordered.append(_section('Debit Card Take Rate',
                                subtitle=section_subtitle))
        ordered.extend(dctr_main)
        ordered.extend(value_dctr)

    if rege_main or value_reg_e:
        ordered.append(_section('Reg E Analysis',
                                subtitle=section_subtitle))
        ordered.extend(rege_main)
        ordered.extend(value_reg_e)

    if attrition_main:
        ordered.append(_section('Account Attrition',
                                subtitle=section_subtitle))
        ordered.extend(attrition_main)

    # Summary placeholder after analysis, before appendix
    ordered.append(_section('Summary & Key Takeaways',
                            layout_index=12, slide_type='blank'))

    # Appendix: divider + older mailer months + overview + analysis detail
    has_appendix = (appendix_clusters or dctr_appendix or rege_appendix
                     or attrition_appendix or overview)
    if has_appendix:
        ordered.append(_section('Appendix',
                                subtitle=section_subtitle))
...
def step_build_deck(ctx):
    ...
    for s in ordered:
        if s.get('__section__'):
            sc = SC(slide_type=s['slide_type'], title=s['title'],
                    layout_index=s['layout_index'], kpis=s.get('kpis', {}))
    ...
    if os.path.exists(pptx_path):
        ...
        if lock_file.exists():
            _report(ctx, f"❌ Cannot write — {os.path.basename(pptx_path)} is open in another application. Close it and retry.")
    ...
    for i, s in enumerate(final_slides):
        if s.layout_index > _max_layout:
            print(f"🔴 BAD LAYOUT: slide {i} '{s.title}' type='{s.slide_type}' layout_index={s.layout_index} (max={_max_layout})")
        else:
...
def run_a1(ctx):
    ...
    label_map = {'Yes': 'Business', 'No': 'Personal', 'Y': 'Business', 'N': 'Personal', '': 'Unknown', 'Unknown': 'Unknown'}

    for sc in grouped['Stat Code'].unique():
        rows = grouped[grouped['Stat Code'] == sc]
        st = rows['Total Count'].sum()
        output_data.append({'Stat Code': sc, 'Account Type': 'All', 'Total Count': st, 'Percent of Stat': st/total})
        biz, pers = 0, 0
        for _, r in rows.iterrows():
            lbl = label_map.get(str(r['Business?']).strip(), str(r['Business?']))
            cnt = r['Total Count']
            if lbl == 'Business': biz = cnt
            elif lbl == 'Personal': pers = cnt
            output_data.append({'Stat Code': sc, 'Account Type': f"  → {lbl}", 'Total Count': cnt, 'Percent of Stat': cnt/st})
        summary_data.append({'Stat Code': sc, 'Total Count': st, 'Percent of Total': st/total, 'Business Count': biz, 'Personal Count': pers})
    ...
    save_to_excel(ctx, df={'Distribution': a1_dist, 'Summary': a1_summary},
        sheet_name='A1-StatCode', analysis_title='Stat Code Distribution Analysis',
        key_metrics={'Total Accounts': f"{len(data):,}", 'Stat Codes': len(a1_summary),
    ...
    try:
        ...
        ax.set_xlabel('Count'); ax.set_title('Stat Code Distribution - Top 10')
        ax.xaxis.set_major_formatter(FuncFormatter(lambda x, p: f'{x:,.0f}'))
        for i, (c, p) in enumerate(zip(dp['Total Count'], dp['Percent of Total'])):
            ax.text(c + dp['Total Count'].max()*0.01, i, f'{c:,.0f} ({p:.1%})', va='center', fontsize=9)
        plt.tight_layout()
    ...
    insight = {
        ...
        'layout_index': 4,
        'insights': [f"Top stat code '{tc}': {tp:.1%}", f"Top 3: {t3:.1%}",
                     f"Personal: {tp_cnt:,}  |  Business: {tb_cnt:,}"],
        'chart_path': chart_path, 'category': 'Overview'
    }
    if chart_path:
...
def run_a2(ctx):
    ...
    total_all = len(data)
    eligible = data[
        (data['Stat Code'].isin(ctx['eligible_stat_code'])) & (data['Prod Code'].isin(ctx['eligible_prod_code'])) &
        (data['Mailable?'].isin(em)) & data['Date Closed'].isna() & (data['Avg Bal'] != 0)
    ]
    te = len(eligible)
    if te == 0:
        _report(ctx, "   ⚠️ No eligible accounts"); return ctx

    ec = eligible.copy()
    ec['Debit?'] = ec['Debit?'].astype(str).str.strip()
    ec['Business?'] = ec['Business?'].astype(str).str.strip()

    wd = len(ec[ec['Debit?'] == 'Yes']); wod = len(ec[ec['Debit?'] == 'No'])
    dc = ec[ec['Debit?'] == 'Yes']; pwd = len(dc[dc['Business?'] == 'No']); bwd = len(dc[dc['Business?'] == 'Yes'])
    ...
    di = {'eligible_pct': te/total_all*100, 'penetration': wd/te*100,
          'personal_pct': pwd/wd*100 if wd else 0, 'business_pct': bwd/wd*100 if wd else 0}

    save_to_excel(ctx, df=a2_summary, sheet_name='A2-DebitCard',
        analysis_title='Debit Card Summary Analysis',
        key_metrics={'Total': f"{total_all:,}", 'Eligible': f"{di['eligible_pct']:.1f}%", 'Penetration': f"{di['penetration']:.1f}%"})
    ...
    try:
        ...
        ax.set_xlabel('Count'); ax.xaxis.set_major_formatter(FuncFormatter(lambda x, p: f'{x:,.0f}'))
        for i, (c, p) in enumerate(zip(pd2['Count'], pd2['Percentage'])):
            ax.text(c + pd2['Count'].max()*0.01, i, f'{c:,.0f} ({p:.1%})', va='center', fontsize=9)
        plt.tight_layout()
    ...
    insight = {
        ...
        'kpis': {'Penetration': f"{di['penetration']:.1f}%", 'Eligible': f"{di['eligible_pct']:.1f}%"},
        'layout_index': 5, 'chart_path': chart_path, 'category': 'Overview'
    }
    if chart_path:
...
def run_a3(ctx):
    ...
    _report(ctx, "\n🎯 A3 — Eligibility Funnel")
    data = ctx['data']; chart_dir = ctx['chart_dir']
    oa = ctx['open_accounts']; ed = ctx['eligible_data']
    ep = ctx['eligible_personal']; eb = ctx['eligible_business']
    esc = ctx['eligible_stat_code']; epc = ctx['eligible_prod_code']
    em = ctx['eligible_mailable'] if isinstance(ctx['eligible_mailable'], list) else [ctx['eligible_mailable']]

    ta = len(data); oc = len(oa)
    sc = len(oa[oa['Stat Code'].isin(esc)]); se = oa[oa['Stat Code'].isin(esc)]
    pc = len(se[se['Prod Code'].isin(epc)]); pe = se[se['Prod Code'].isin(epc)]
    mc = len(pe[pe['Mailable?'].isin(em)]); ec = len(ed)

    funnel = pd.DataFrame([
        {'Stage': '1. Total Accounts', 'Count': ta, 'Pct of Total': 1.0, 'Drop-off': 0, 'Drop-off %': 0.0},
        {'Stage': '2. Open Accounts', 'Count': oc, 'Pct of Total': oc/ta, 'Drop-off': ta-oc, 'Drop-off %': (ta-oc)/ta},
        {'Stage': '3. + Eligible Status Code', 'Count': sc, 'Pct of Total': sc/ta, 'Drop-off': oc-sc, 'Drop-off %': (oc-sc)/oc if oc else 0},
        {'Stage': '4. + Eligible Product Code', 'Count': pc, 'Pct of Total': pc/ta, 'Drop-off': sc-pc, 'Drop-off %': (sc-pc)/sc if sc else 0},
        {'Stage': '5. + Mailable', 'Count': mc, 'Pct of Total': mc/ta, 'Drop-off': pc-mc, 'Drop-off %': (pc-mc)/pc if pc else 0},
        {'Stage': '6. + Has Balance (ELIGIBLE)', 'Count': ec, 'Pct of Total': ec/ta, 'Drop-off': mc-ec, 'Drop-off %': (mc-ec)/mc if mc else 0},
    ])
    if ec > 0:
        funnel = pd.concat([funnel, pd.DataFrame([
            {'Stage': '   → Personal', 'Count': len(ep), 'Pct of Total': len(ep)/ec, 'Drop-off': None, 'Drop-off %': None},
            {'Stage': '   → Business', 'Count': len(eb), 'Pct of Total': len(eb)/ec, 'Drop-off': None, 'Drop-off %': None},
        ])], ignore_index=True)
    ...
    save_to_excel(ctx, df=funnel, sheet_name='A3-Eligibility',
        analysis_title='Eligibility Funnel',
        key_metrics={'Total': f"{ta:,}", 'Eligible': f"{ec:,} ({er:.1f}%)", 'Biggest Drop': biggest})
    ...
    try:
        ...
        tdf['Drop-off %'] = tdf['Drop-off %'].apply(lambda x: f"{x:.1%}" if pd.notna(x) and x > 0 else "—")
        t = ax.table(cellText=tdf.values, colLabels=tdf.columns, cellLoc='center', loc='center',
    ...
    insight = {
        ...
        'layout_index': 9, 'kpis': {'Eligible': f"{er:.1f}%", 'Accounts': f"{ec:,}"},
        'chart_path': chart_path, 'category': 'Overview'
    }
    if chart_path:
        add_to_library(ctx, 'A3 - Eligibility Funnel', insight, 'Overview')

    ctx['results']['a3'] = {'funnel': funnel, 'insight': insight,
        'insights': {'total_accounts': ta, 'eligible_accounts': ec, 'eligibility_rate': er,
                     'biggest_dropoff': biggest, 'personal_pct': len(ep)/ec*100 if ec else 0,
...
def run_a4(ctx):
    ...
    _report(ctx, "\n📅 A4 — Monthly Trends")
    ed = ctx['eligible_data']; chart_dir = ctx['chart_dir']

    if 'Date Opened' not in ed.columns:
        _report(ctx, "   ⚠️ No Date Opened column"); return ctx

    ed_copy = ed.copy()
    ed_copy['Date Opened'] = pd.to_datetime(ed_copy['Date Opened'], errors='coerce')
    valid = ed_copy[ed_copy['Date Opened'].notna()]
    if valid.empty:
        _report(ctx, "   ⚠️ No valid dates"); return ctx

    valid['Year'] = valid['Date Opened'].dt.year
    monthly = valid.pivot_table(index='Year', columns=valid['Date Opened'].dt.month, aggfunc='size', fill_value=0)
    ...
    save_to_excel(ctx, df=trends, sheet_name='A4-Trends',
    ...
    try:
        ...
        ax.set_ylabel('Openings'); ax.set_title('Account Openings by Year')
        ...
        if ny > 10: plt.xticks(rotation=45)
        plt.tight_layout()
    ...
    insight = {'title': 'Account Opening Trends',
        'subtitle': f"{gt:,} eligible accounts across {ny} years",
        'layout_index': 9, 'kpis': {'Openings': f"{gt:,}", 'Years': f"{ny}"},
        'chart_path': chart_path, 'category': 'Overview'}
    if chart_path:
...
def run_a5(ctx):
    ...
    for bn in sorted(ec['Branch Name'].unique()):
        ...
        rows.append({'Branch': bn, 'Total': len(ba),
    ...
    total_row = {'Branch': 'TOTAL', 'Total': a5['Total'].sum(),
        'Personal': a5['Personal'].sum(), 'Business': a5['Business'].sum(), 'Pct of Total': 1.0}
    a5 = pd.concat([a5, pd.DataFrame([total_row])], ignore_index=True)

    dr = a5[a5['Branch'] != 'TOTAL']
    nb = len(dr); lb = dr.iloc[0]['Branch']; lp = dr.iloc[0]['Pct of Total']*100

    save_to_excel(ctx, df=a5, sheet_name='A5-Branch-Dist',
    ...
    try:
        ...
        ax.set_xlabel('Count'); ax.set_title('Branch Distribution - Top 10')
        ax.xaxis.set_major_formatter(FuncFormatter(lambda x, p: f'{x:,.0f}'))
    ...
    insight = {'title': 'Branch Distribution',
        'subtitle': f"{lb} leads with {lp:.0f}% — {nb} branches total",
        'layout_index': 4, 'kpis': {'Branches': f"{nb}", 'Top': lb},
        'chart_path': chart_path, 'category': 'Overview'}
    if chart_path:
...
def run_pipeline(file_path, config_path=None, base_paths=None,
    ...
    for run_fn, label in [(run_a1, 'A1'), (run_a2, 'A2'), (run_a3, 'A3'),
                           (run_a4, 'A4'), (run_a5, 'A5')]:
        try:
            ctx = run_fn(ctx)
        except Exception as e:
