...
def _draw_comparison_table(ctx, row_data, col1_header, col2_header,
                           col1_color='#6B7F99', col2_color='#4A9B9F',
    ...
    # Column headers
    ax.text(5, 9, col1_header, fontsize=18, fontweight='500',
            color='#333333', ha='center')
    ax.text(7.5, 9, col2_header, fontsize=18, fontweight='500',
            color='#333333', ha='center')

    n_rows = len(row_data)
    for i, (y_pos, label, with_val, without_val) in enumerate(row_data):
        # Row label
        ax.text(2.5, y_pos, label, fontsize=16, color='#333333',
                va='center', ha='right')
        ...
        # Column-1 cell
        rect1 = Rectangle((3.75, y_pos - 0.5), 2.25, 1,
                           facecolor=c1, edgecolor='none')
        ax.add_patch(rect1)
        ax.text(5, y_pos, with_val, fontsize=18, color='white',
                ha='center', va='center', fontweight='500')

        # Column-2 cell
        rect2 = Rectangle((6.25, y_pos - 0.5), 2.25, 1,
                           facecolor=c2, edgecolor='none')
        ax.add_patch(rect2)
        ax.text(7.5, y_pos, without_val, fontsize=18, color='white',
                ha='center', va='center', fontweight='500')

        # Dotted line separator (skip after last row)
        if i < n_rows - 1:
            ax.plot([2.5, 8.5], [y_pos - 0.75, y_pos - 0.75],
...
def _draw_value_slide(ctx, row_data, col1_header, col2_header, impact,
                      col1_color='#6B7F99', col2_color='#4A9B9F',
                      highlight_color='#D4A574'):
...
def run_value_1(ctx):
    ...
    if ep is None or ep.empty:
        _report(ctx, "   ⚠️ No eligible personal accounts"); return ctx
    ...
    if 'Yes' not in rev.index or 'No' not in rev.index:
        _report(ctx, "   ⚠️ Need both Yes and No debit groups"); return ctx
    ...
    row_data = [
        ...
        (6.5, f'NSF/OD Revenue\n(${fee_amount})',
             f'${nsf_w:,.0f}', f'${nsf_wo:,.0f}'),
        (5,   f'Interchange\nRevenue ({rate_amount:.4f})',
             f'${ic_w:,.0f}', f'${ic_wo:,.0f}'),
        (3.5, 'Total Revenue',
             f'${rw:,.0f}', f'${rwo:,.0f}'),
        (2,   'Revenue Per\nAccount',
             f'${rpw:.2f}', f'${rpwo:.2f}'),
    ...
    try:
        impact = {
            ...
            'pot_hist': pot_hist, 'pot_l12m': pot_l12m,
            'pot_100': pot_100,
            'rate_label': 'DCTR',
        }
        ...
        subtitle = (f"${delta:.2f} more revenue per account — "
                    f"${pot_l12m:,.0f} potential at {l12m_dctr:.0%} TTM DCTR")
        _slide(ctx, 'A11.1 - Value of a Debit Card', {
    ...
    _save(ctx, comp_df, 'A11-DebitCardValue',
    ...
    _save(ctx, summary_df, 'A11-ValueSummary',
          'Value of a Debit Card - Summary Metrics')

    ctx['results']['value_1'] = {
        'delta': delta, 'multiplier': multiplier,
        'accts_with': aw, 'accts_without': awo,
        'rev_per_with': rpw, 'rev_per_without': rpwo,
        'nsf_w': nsf_w, 'nsf_wo': nsf_wo,
        'ic_w': ic_w, 'ic_wo': ic_wo,
        'rev_with': rw, 'rev_without': rwo,
...
def run_value_2(ctx):
    ...
    if not lrc or lrc not in base.columns:
        _report(ctx, f"   ⚠️ Reg E column not found — skipping A11.2")
        _report(ctx, f"   ctx key: {ctx.get('latest_reg_e_column')}")
        _report(ctx, f"   config key: {ctx.get('config', {}).get('LatestRegEColumn')}")
        _report(ctx, f"   Available cols with 'Reg': {[c for c in base.columns if 'reg' in c.lower()][:5]}")
        ctx['results']['value_2'] = {}
        return ctx
    ...
    items_col = _find_col(active, 'items')
    if not spend_col or not items_col:
        avail = [c for c in active.columns if any(k in c.lower() for k in ['spend', 'item', 'txn', 'trans', 'amount', 'revenue'])]
        _report(ctx, f"   ⚠️ Could not find L12M spend/items columns — skipping A11.2")
    ...
    if True not in rev.index or False not in rev.index:
        _report(ctx, "   ⚠️ Need both opted-in and opted-out groups"); return ctx
    ...
    try:
        ...
        _slide(ctx, 'A11.2 - Value of Reg E Opt-In', {
    ...
    _save(ctx, comp_df, 'A11.2-RegEValue',
