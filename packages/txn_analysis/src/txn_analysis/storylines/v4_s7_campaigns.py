...
def _classify_responders(offer_accounts, resp_col, offer_val):
    ...
    if "NU" in str(offer_val):
        mask = offer_accounts[resp_col].notna() & ~offer_accounts[resp_col].astype(
            str
        ).str.contains(r"NU [1-4]", na=False, regex=True)
    else:
...
def _campaign_overview(odd):
    ...
    resp_mask = (
        odd["# of Responses"] > 0
        if "# of Responses" in odd.columns
        else pd.Series(False, index=odd.index)
    )
    ...
    ax = fig.get_axes()[0]
    set_insight_title(
        ax, f"{rate:.1f}% overall campaign response rate", f"{rc:,} responders out of {mc:,} mailed"
    )
    narr = (
...
def _response_by_generation(odd):
    ...
    with chart_figure(figsize=(10, 5)) as (fig, ax):
        x_pos = range(len(gs))
        ax.bar(x_pos, gs["Response Rate (%)"], color=colors)
        for i, val in enumerate(gs["Response Rate (%)"]):
            ax.annotate(
                f"{val:.1f}%",
                xy=(i, val),
                xytext=(0, 4),
        ...
        ax.set_xticklabels(gs["Generation"].tolist())
        ax.set_ylabel("Response Rate (%)")
        set_insight_title(ax, "Campaign response rate by generation")
        fig.tight_layout()
...
def _spend_lift(odd):
    ...
    with chart_figure(figsize=(10, 5)) as (fig, ax):
        ...
        ax.bar(x_pos, combined["Avg Spend"], color=CATEGORY_PALETTE[: len(combined)])
        for i, val in enumerate(combined["Avg Spend"]):
            ax.annotate(
                format_currency(val),
                xy=(i, val),
                xytext=(0, 4),
        ...
        ax.set_xticks(list(x_pos))
        ax.set_xticklabels(combined["Group"].tolist())
        ax.set_ylabel("Avg Total Spend")
        set_insight_title(ax, "Average spend: responders vs non-responders")
        fig.tight_layout()
...
def _monthly_tracking(odd):
    ...
    with chart_figure(figsize=(10, 5)) as (fig, ax):
        x_pos = range(len(tdf))
        ax.bar(x_pos, tdf["Mailed"], color=COLORS["primary"], alpha=0.7, label="Mailed")
        ax.set_xticks(list(x_pos))
        ax.set_xticklabels(tdf["Month"].tolist(), rotation=-45, ha="right")
        ax.set_ylabel("Accounts Mailed")

        ax2 = ax.twinx()
        ax2.plot(
            list(x_pos),
            tdf["Response Rate (%)"].tolist(),
            color=COLORS["accent"],
            linewidth=3,
            marker="o",
            markersize=8,
        ...
        ax2.set_ylim(0, max(tdf["Response Rate (%)"].max() * 1.3, 10))
        ...
        set_insight_title(ax, "Monthly campaign cadence & response rate")
        fig.tight_layout()
...
def _segmentation_performance(odd):
    ...
    ax = fig.get_axes()[0]
    set_insight_title(ax, f"Segmentation performance for {label}", f"{len(st)} segments analyzed")
...
def _response_by_balance_tier(odd):
    ...
    with chart_figure(figsize=(10, 5)) as (fig, ax):
        x_pos = np.arange(len(ct_pct))
        ax.bar(x_pos, ct_pct["Responder"], color=COLORS["positive"], label="Responder")
        ax.bar(
            x_pos,
            ct_pct["Non-Responder"],
            bottom=ct_pct["Responder"],
            color=COLORS["neutral"],
            label="Non-Responder",
        )
        ax.set_xticks(x_pos)
        ax.set_xticklabels(ct_pct["balance_tier"].tolist())
        ax.set_ylim(0, 100)
        ...
        set_insight_title(ax, "Response rate by balance tier")
        fig.tight_layout()
...
def _offer_lift(odd):
    ...
    narr = f"Across {len(mail_cols)} campaigns, <b>{len(agg)}</b> offer types analyzed. "
...
def _before_after_trends(odd):
    ...
    for m in window:
        ...
        rows.append(
            {
                "Month": m,
                "Responder Avg Spend": round(r_avg, 2),
    ...
    with chart_figure(figsize=(10, 5)) as (fig, ax):
        ...
        if camp_pos is not None:
            ax.axvline(x=camp_pos, linestyle="--", color=COLORS["negative"], label="Campaign Month")

        ax.set_xticks(x)
        ax.set_xticklabels(tdf["Month"].tolist())
...
def _txn_size_buckets(odd):
    ...
    resp, non_resp = _classify_responders(
        campaign_accts, rc, campaign_accts[latest_mail].iloc[0] if not campaign_accts.empty else ""
    )
...
def _offer_txn_detail(odd):
    ...
    ax = fig.get_axes()[0]
    set_insight_title(
        ax, "Swipe behavior by offer type", "Responders consistently swipe more often"
    )
...
def _biz_personal_campaigns(odd):
    ...
    grp = (
        offered.groupby("Business?")
        .agg(
            mailed=("# of Offers", "count"),
            responders=("# of Responses", lambda s: (s > 0).sum()),
            avg_spend=("Total Spend", "mean")
            if "Total Spend" in offered.columns
            else ("# of Offers", "count"),
        )
        .reset_index()
    )
    ...
    grp["Account Type"] = (
        grp["Account Type"].map({"Yes": "Business", "No": "Personal"}).fillna(grp["Account Type"])
    )
    ...
    ax = fig.get_axes()[0]
    set_insight_title(
        ax, "Business vs personal campaign response", f"{len(grp)} account types compared"
    )
...
def run(ctx: dict) -> dict:
    ...
    sections.append(
        {
            ...
            "narrative": ov_narr,
            "figures": [ov_fig],
            "tables": [("Campaign Overview", ov_df)],
        }
    )
    sheets.append(
        {
            "name": "S7 Campaign Overview",
            "df": ov_df,
            "currency_cols": [],
            "pct_cols": [],
            "number_cols": ["Value"],
    ...
    _add(
        sections,
        sheets,
        "Response Rate by Generation",
        *_response_by_generation(odd),
        "S7 Response by Gen",
        currency_cols=[],
        pct_cols=["Response Rate (%)"],
    ...
    _add(
        sections,
        sheets,
        "Spend Lift: Responders vs Non-Responders",
        *_spend_lift(odd),
        "S7 Spend Lift",
        currency_cols=["Avg Spend"],
        pct_cols=[],
    ...
    _add(
        sections,
        sheets,
        "Monthly Mail & Response Tracking",
        *_monthly_tracking(odd),
        "S7 Monthly Tracking",
        currency_cols=[],
        pct_cols=["Response Rate (%)"],
    ...
    _add(
        sections,
        sheets,
        "Response by Balance Tier",
        *_response_by_balance_tier(odd),
        "S7 Balance Tier Resp",
        currency_cols=[],
        pct_cols=["Response Rate (%)"],
    ...
    _add(
        sections,
        sheets,
        "Before/After Campaign Spending Trends",
        *_before_after_trends(odd),
        "S7 Before After",
    ...
    _add(
        ...
        *_txn_size_buckets(odd),
        "S7 Txn Buckets",
        currency_cols=[],
        pct_cols=["Responders (%)", "Non-Responders (%)"],
    ...
    _add(
        sections,
        sheets,
        "Transaction Detail by Offer Type",
        *_offer_txn_detail(odd),
        "S7 Offer Txn Detail",
        currency_cols=["Resp Avg Txn ($)", "Non-Resp Avg Txn ($)"],
        pct_cols=[],
        number_cols=["Resp Avg Swipes", "Non-Resp Avg Swipes"],
    )
    ...
    _add(
        sections,
        sheets,
        "Response Rate by Age & Tenure",
        *_response_by_age_tenure(odd),
        "S7 Age Tenure Resp",
        currency_cols=[],
        pct_cols=["Response Rate (%)"],
        number_cols=["Mailed", "Responders"],
