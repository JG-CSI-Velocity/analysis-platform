...
def overview_ctx(overview_df, tmp_output_dir):
    ...
    return PipelineContext(
        client=ClientInfo(
            client_id="1200", client_name="Test CU", month="2026.02",
...
def dctr_ctx(dctr_eligible_df, tmp_output_dir):
    ...
    return PipelineContext(
        client=ClientInfo(
            client_id="1200", client_name="Test CU", month="2024.02",
            eligible_stat_codes=["O"],
        ),
        paths=paths,
        data=df,
...
def mailer_ctx(mailer_df, tmp_output_dir):
    ...
    return PipelineContext(
        client=ClientInfo(
            client_id="9999", client_name="Test CU", month="2024.05",
            eligible_stat_codes=["O"],
            ic_rate=0.0015,
        ),
        paths=paths,
...
def attrition_df():
    ...
    dates_closed = (
        [pd.NaT] * 30
        + list(pd.date_range("2023-01-31", periods=30, freq="ME"))
    )
    return pd.DataFrame({
...
def insights_ctx(tmp_output_dir):
    ...
    ctx.results["value_1"] = {
        "delta": 85.50, "accts_with": 1200, "accts_without": 800,
        "rev_per_with": 142.30, "rev_per_without": 56.80,
        "hist_dctr": 0.60, "l12m_dctr": 0.65,
        "pot_hist": 41040.0, "pot_l12m": 44460.0, "pot_100": 68400.0,
    }
    ctx.results["value_2"] = {
        "delta": 45.20, "accts_with": 900, "accts_without": 300,
        "rev_per_with": 98.50, "rev_per_without": 53.30,
        "hist_rege": 0.75, "l12m_rege": 0.78,
        "pot_hist": 10170.0, "pot_l12m": 10576.80, "pot_100": 13560.0,
    }
    ctx.results["attrition_1"] = {
        "overall_rate": 0.12, "l12m_rate": 0.10, "total": 2000, "closed": 240,
    }
    ...
    ctx.results["dctr_1"] = {
        "insights": {
            "overall_dctr": 0.60, "recent_dctr": 0.65, "total_accounts": 2000,
        },
    }
    ctx.results["dctr_3"] = {
    ...
    ctx.results["dctr_9"] = {
        "all": {
            "total_branches": 5,
            "best_branch": "Downtown", "best_dctr": 0.78,
            "worst_branch": "Rural", "worst_dctr": 0.42,
        },
    }
    ctx.results["reg_e_1"] = {
        "opt_in_rate": 0.75, "l12m_rate": 0.78,
        "total_base": 1200, "opted_in": 900, "opted_out": 300,
    }
    ctx.results["market_reach"] = {
        "n_eligible": 1500, "n_responders": 350,
        "n_mailed": 800, "penetration": 23.3,
    }
    ctx.results["revenue_attribution"] = {
        "resp_ic": 4200.0, "non_ic": 2800.0, "incremental_total": 1400.0,
    }
    ctx.results["pre_post_delta"] = {
        "resp_pre": 420.0, "resp_post": 510.0, "resp_delta": 90.0,
        "non_pre": 380.0, "non_post": 395.0, "non_delta": 15.0,
    }
    ctx.results["a3"] = {
        "insights": {
            "total_accounts": 2500, "eligible_accounts": 2000,
